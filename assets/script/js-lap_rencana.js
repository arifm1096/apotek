...
function load_rencana(tgl1, tgl2) {
	$("#tbl_rencana").DataTable({
		ajax: {
			url: URL + "laporan/load_beli_rencana",
			type: "POST",
...
function filter_data() {
	...
	if (tgl1 !== "" && tgl2 !== "") {
		$("#tbl_rencana").DataTable().destroy();
		load_rencana(tgl1, tgl2);
	}
}

function clear_filter() {
	$("#tbl_rencana").DataTable().destroy();
	load_rencana((text = ""), (tgl = ""));
	$("#filter_text").val("");
	$("#tanggal").val("");
}
